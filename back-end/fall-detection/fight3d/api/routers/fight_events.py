...
router = APIRouter(prefix="/api/fall-events", tags=["fall-events"])


@router.post("/batch", response_model=BatchIngestResult)
async def post_fall_events_batch(
    request: Request,
    ...
    async with session.begin():
        for ev in payload.events:
            ...
            if was_upsert:
                ...
                # Emit fall.detected once per newly inserted open fall.
                is_open = (ev.ended_at is None) and (ev.status in (None, "open"))
                if is_open:
                    meta = ev.meta or {}
                    age_raw = meta.get("age") or 0
                    try:
                        age = int(age_raw)
                    except Exception:
                        age = 0
                    ...
                    to_broadcast.append(
                        {
                            ...
                            "payload": {
                                ...
                                "person_type": str(meta.get("person_type") or "unknown"),
                                "age": age,
                            },
                        }
                    )
