...
from fight3d.utils.viz import DEFAULT_FALL_THRESHOLD, draw_boxes, put_status_line
...
def main() -> None:
    ...
    viz.DEFAULT_FALL_THRESHOLD = float(args.fall_threshold)
    ...
    while True:
        ...
        if people_count < int(args.people_min):
            ...
            if len(buf) == int(args.clip_len) and (frame_idx % int(args.stride) == 0):
                ...
                state = "FALL" if score >= float(args.fall_threshold) else "IDLE"

                if state == "FALL" and not last_event_fall:
                    print(f"[FALL] t={ts:.2f}s score={score:.3f} people={people_count}")
