...
class _FrameReader:
    """Background thread that continuously drains the cap buffer and exposes only the
    latest frame.  This prevents RTSP latency from accumulating when inference is
    slower than the camera FPS.
    """

    def __init__(self, cap: cv2.VideoCapture) -> None:
        self._cap = cap
        self._q: queue.Queue[tuple[bool, np.ndarray | None]] = queue.Queue(maxsize=1)
    ...
    def _loop(self) -> None:
        while not self._stop.is_set():
            ...
            if not ok:
                # Signal failure to the consumer and exit.
                try:
                    self._q.get_nowait()
                except queue.Empty:
                    pass
                ...
                return
            # Discard stale frame so the consumer always gets the freshest one.
            try:
                self._q.get_nowait()
            except queue.Empty:
                pass
    ...
    def read(self) -> tuple[bool, np.ndarray | None]:
        """Block until the next fresh frame (or a failure signal) is available."""
        try:
    ...
    def stop(self) -> None:
        self._stop.set()
        # Drain the queue so the loop thread can unblock if it is waiting on put.
        try:
            self._q.get_nowait()
        except queue.Empty:
            pass
...
def _fall_score(model: torch.nn.Module, x: torch.Tensor) -> float:
    logits = model(x)
...
def run_rtsp(
    *,
    ...
    print(
        ...
        f"fall_threshold={float(fall_threshold)} people_min={int(people_min)} clip_len={int(clip_len)} stride={int(stride)}",
        flush=True,
    )
    ...
    # On Windows, DISPLAY/WAYLAND_DISPLAY are not used — GUI is always available natively.
    ...
    viz.DEFAULT_FALL_THRESHOLD = float(fall_threshold)
    ...
    if use_fp16:
        model.half()
        print(f"[FP16] enabled src={window_name}", flush=True)
    ...
    fall_writer = None
    fall_save_path_final: Path | None = None
    fall_save_path_part: Path | None = None

    def _rec_finalize() -> None:
        nonlocal fall_writer, fall_save_path_final, fall_save_path_part
        if fall_writer is None:
            return
        try:
            fall_writer.release()
        finally:
            fall_writer = None
        if fall_save_path_part is not None and fall_save_path_final is not None:
            if fall_save_path_part.exists():
                try:
                    os.replace(str(fall_save_path_part), str(fall_save_path_final))
                    print(f"[REC] stop src={window_name} path={fall_save_path_final}", flush=True)
                except Exception as e:
                    print(
                        f"[REC] stop src={window_name} path={fall_save_path_final} err={e}",
                        flush=True,
                    )
            else:
                print(
                    f"[REC] stop src={window_name} path={fall_save_path_final} err=missing_temp_file temp={fall_save_path_part}",
                    flush=True,
                )
        fall_save_path_final = None
        fall_save_path_part = None

    buf: deque[np.ndarray] = deque(maxlen=int(clip_len))
    frame_idx = 0
    state = "IDLE"
    score: float | None = None
    last_event_fall = False

    # Optional Events API reporting (writes fall events to PostgreSQL via Events API).
    ...
    fall_opened_at: datetime | None = None
    fall_dedup_key: str | None = None
    fall_peak: float = 0.0
    fall_sum: float = 0.0
    fall_cnt: int = 0

    def _post_fall_event(
        *,
        started_at: datetime,
        ...
        url = f"{events_api}/api/fall-events/batch"
        try:
            req = Request(
                url,
    ...
    def _close_open_fall_segment(reason: str) -> None:
        nonlocal fall_opened_at, fall_dedup_key, fall_peak, fall_sum, fall_cnt
        if not events_enabled:
            return
        if fall_opened_at is None:
            return
        ended_at = datetime.now(timezone.utc)
        try:
            _post_fall_event(
                started_at=fall_opened_at,
                ended_at=ended_at,
                peak=fall_peak,
                mean=fall_sum / max(1, fall_cnt),
                status="closed",
            )
        finally:
            fall_opened_at = None
    ...
    while True:
        ...
        while True:
            ...
            ok, frame = reader.read() if cap is not None else (False, None)
            ...
            if people_count < int(people_min):
                ...
                if len(buf) == int(clip_len) and (frame_idx % int(stride) == 0):
                    ...
                    if use_fp16:
                        x = x.half()
                    score = _fall_score(model, x)
                    state = "FALL" if score >= float(fall_threshold) else "IDLE"

                    if state == "FALL" and not last_event_fall:
                        print(
                            f"[FALL] src={window_name} t={ts:.2f}s score={score:.3f} people={people_count}",
                            flush=True,
                        )
                        last_event_fall = True
                    if state != "FALL":
                        last_event_fall = False
            ...
            if events_enabled:
                ...
                if state == "FALL":
                    if fall_opened_at is None and score is not None:
                        fall_opened_at = now
                        fall_dedup_key = f"{camera_external_id}:{fall_opened_at.isoformat()}"
                        fall_peak = float(score)
                        fall_sum = float(score)
                        fall_cnt = 1
                        _post_fall_event(
                            started_at=fall_opened_at,
                            ended_at=None,
                    ...
                    elif fall_opened_at is not None and score is not None:
                        fall_peak = max(float(score), fall_peak)
                        fall_sum += float(score)
                        fall_cnt += 1
                else:
                    if fall_opened_at is not None:
            ...
            # Record video segments on fall start/end when save is a directory.
            if save_dir is not None:
                if state == "FALL":
                    if fall_writer is None:
                        ...
                        fall_save_path_final = save_dir / f"{cam}-{ts_name}.mp4"
                        # IMPORTANT: keep a real .mp4 extension so OpenCV can select the MP4 muxer.
                        fall_save_path_part = save_dir / f"{cam}-{ts_name}.part.mp4"
                        try:
                            if fall_save_path_part.exists():
                                fall_save_path_part.unlink()
                        except Exception:
                            pass
                        ...
                        hh, ww = vis.shape[:2]
                        fourcc = cv2.VideoWriter_fourcc(*"mp4v")
                        fall_writer = cv2.VideoWriter(str(fall_save_path_part), fourcc, float(fps_out), (ww, hh))
                        if not fall_writer.isOpened():
                            print(
                                f"[REC] start failed src={window_name} path={fall_save_path_final} temp={fall_save_path_part}",
                                flush=True,
                            )
                            try:
                                fall_writer.release()
                            ...
                            print(
                                f"[REC] start src={window_name} path={fall_save_path_final}",
                                flush=True,
                            )
                    if fall_writer is not None:
                        fall_writer.write(vis)
                else:
                    _rec_finalize()
            ...
            if draw_state == "FALL":
                ...
                vis = draw_boxes(vis, [union] if union is not None else [], state="FALL", score=draw_score)
            else:
            ...
            status = f"fps={fps_smooth:.1f}  state={state}  fall_score={score_txt}  people={people_count}"
            if paused:
                status += "  PAUSED"
        ...
        try:
            if cap is not None:
                cap.release()  # Unblocks any blocking cap.read() in _FrameReader thread
        finally:
            cap = None
        ...
        _rec_finalize()
        _close_open_fall_segment("disconnect")
    ...
    if writer is not None and not isinstance(writer, tuple):
        writer.release()
    _rec_finalize()
    _close_open_fall_segment("shutdown")
    if show == 1:
