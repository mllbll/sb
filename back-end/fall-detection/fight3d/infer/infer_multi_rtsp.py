...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--fall_threshold", type=float, default=None)
...
def _load_sources(args: argparse.Namespace, settings) -> list[tuple[str, str]]:
    ...
    if not sources and settings is not None:
        for cam in settings.cameras:
            sources.append((cam.name, cam.rtsp, cam.fall_threshold))
...
def _start_proc(*, ctx: mp.context.BaseContext, spec: _WorkerSpec, args: argparse.Namespace) -> mp.Process:
    p = ctx.Process(
        ...
        kwargs={
            ...
            "fall_threshold": spec.fall_threshold,
        },
        daemon=False,
    )
    p.start()
...
def main() -> None:
    ...
    if settings is not None:
        ...
        if args.fall_threshold is None:
            args.fall_threshold = inf.fall_threshold
    ...
    for idx, (name, rtsp, ft_override) in enumerate(sources):
        ...
        ft = float(ft_override) if ft_override is not None else float(args.fall_threshold)
        specs.append(
            _WorkerSpec(
                idx=idx,
                name=name,
