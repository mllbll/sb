...
def load_settings(path: str | None = None) -> Settings:
    ...
    for i, item in enumerate(cameras_raw):
        ...
        ft_raw = item.get("fall_threshold", None)
        fall_threshold = None
        if ft_raw is not None and str(ft_raw).strip() != "":
            try:
                fall_threshold = float(ft_raw)
            except Exception as e:
                raise ValueError(
                    f"config.yaml: cameras[{i}].fall_threshold must be a number"
                ) from e

        cameras.append(CameraSettings(name=name, rtsp=rtsp, fall_threshold=fall_threshold))
    ...
    inference = InferenceSettings(
        ...
        fall_threshold=float(inf_raw.get("fall_threshold") or 0.7),
